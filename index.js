...
function getFinals(data) {
    /* code here */
    const allFinals = data.filter(function(data){
...
function getWinnersByYear(cbWinner, cbYear) {
    ...
    let winners = cbWinner(getFinals);
    let years = cbYear(getFinals);

    const list = info.map(function(item, index){
...
function getAverageGoals(data) {
    ...
    const homeAvr = (homeGoals / data.length);

    const awayGoals = data.reduce(function(accumulator, item){
        return accumulator + item["Away Team Goals"];
    }, 0);
    const awayAvr = (awayGoals / data.length);

    return `The home team goal average is ${homeAvr} and the away team goal average is ${awayAvr}.`
};
